...
// component root path
const packagePath = 'packages/yike-design-ui'
...
const props = withDefaults(defineProps<${propsName}>(), {
  ...
  const lessContent = `@import '../../styles/basis.less';\n.yk-${componentLowDashName} {
...
function addDemoFile() {
  const exampleBasePath = 'packages/yike-design-demo/src/examples/'
...
function selectAddRouter() {
  const barFile = 'packages/yike-design-demo/src/router/config/components.json'
  fs.readFile(barFile, 'utf8', (err, data) => {
    ...
    let barData
    try {
      barData = JSON.parse(data) // 直接解析成数组
    } catch (e) {
      warn('JSON 解析失败')
      warn(e)
      process.exit(234)
    ...
    rl.question(chalk.yellow('请输入您的选择 (数字)：'), (answer) => {
      ...
      if (isNaN(index) || index < 0 || index >= barData.length) {
        warn('输入不合法，请输入正确的数字')
        rl.close()
        return
      }

      barData[index].list.push({
      ...
      fs.writeFile(barFile, JSON.stringify(barData, null, 2), 'utf8', (err) => {
        ...
        log(`${componentDescName} 组件已添加至 ${barData[index].title} 类别下`)
      })
...
function addRouter() {
  ...
  const generalRouterPath =
    'packages/yike-design-demo/src/router/modules/general.ts'
