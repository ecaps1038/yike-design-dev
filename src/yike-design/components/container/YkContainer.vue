<template>
  <div class="content" @mousemove="moverScroll" @mouseup="mouseup" @mouseleave="mouseup">
    <div ref="boxRef" class="box" @scroll="scrollChange()">
      <div ref="ulRef">
        <slot></slot>
      </div>
    </div>
    <div class="scrollx" v-show="isx && show" :style="{bottom: space+'px',height: size+'px'}">
      <div
        class="scroll-body"
    ...
    <div class="scrolly" v-show="isy && show" :style="{right: space+'px',width: size+'px'}">
      <div
        class="scroll-body"
...
const moverScroll = (e:any) => {
  ...
  if (scrolling.value==0) {
    ...
    nowrollP +((e.screenX - nowscreenX.value) * ulWidth.value) / boxWidth.value;
  }else if (scrolling.value==1) {
    boxRef.value.scrollTop =
    nowrollP +((e.screenY - nowscreenY.value) * ulHeight.value) / boxHeight.value;
  }
};
...
.content {
  ...
  .scrollx {
    ...
    border-radius: 12px;
    opacity: 0;
    transition: opacity @animats;
    // background-color: rgba(#000, 0.1);
    .scroll-body {
      position: absolute;
      left: 0;
      top: 0;
      background-color: @line-color-s;
      width: 100px;
      height: 100%;
      border-radius: 12px;
